...
> 交接时间：2026-09-16
> 项目版本：v2.6.0（功能完整，可正常运行；PyPI 已发布 2.3.1，2.3.2 以后均未发布）
...
**2.6.0（最新）：短语升格为一等学习单位。** 短语从 `phrases` 表并入 `words`
（`type='phrase'`），从此共享遇见跟踪 / 掌握度 / FSRS / 选词 / 图谱节点 —— 与单词同等对待，
但各自成池互不挤占名额。图谱里**词=实心点、短语=空心环**（单色体系下靠形状区分）。
⚠️ 短语库没有释义，要进单词测试得先补。

**2.5.3：知识图谱视觉重做。** 从「四个饱和色相 + 60 个标签 + 直线边 +
玻璃盒边框」改为「单色明度编码掌握度 + 标签默认隐藏 + 弧线淡边 + 柔光节点 + 径向纵深」。
**改图谱视觉前先读 `docs/CHANGELOG.md` 的 [2.5.3]**，里面按感受词做了归因表
（乱/AI 味/不精美/Obsidian 感分别对应什么技术原因），照着改比重新摸索快。

**2.5.2：删除语义全面复查。** 除生词本外又抓到两处：删文章未清
`word_encounters`（造成「幽灵文章」虚高重遇次数，实测积 13 条），删材料未清画像引用；
书架删除补了目录包含校验。

**2.5.1：修复生词本删除捣毁词典条目。** 原来在生词本里点删除会
`DELETE FROM words`，把词从 18.6 万条词典里永久移除（排查中发现 `percent` 就这么没了）。
现在按 `words.source` 分流：词典词只复位成未学状态，只有用户自建的词才真删。

**2.5.0：掌握度模型 + 选词闭环。** 此前项目**没有「掌握」这个概念**。
现在用「识别 / 回忆」两个维度累积证据（依据 Pellicer-Sánchez 2015：8 次语境遇见带来
86% 词形识别 / 75% 意义识别，但回忆只有 55%），选词分四层（到期复习 → 重遇缺口 →
目标考试新词 → 兜底）并排除已能回忆的词。**完整文献依据与参数推导见
`docs/MASTERY-MODEL.md`** —— 改这个模型前必读。

**2.4.1：词汇差距统计。** 统计页新增「词汇差距」区块 —— 词汇量、四级真题覆盖率
（带 i+1 区间标记）、各考试缺口、进度预测。覆盖率**按词频档位给语料加权**而非数词表
（CET4 大纲词表只覆盖四级真题语料 15.8% 的 token）。未定级时用「最高频 2,500 词」的
默认假设并明确标注。另补齐了词典缺失的 11.9% 高频词（关键：`frq` 为 0 时要看 `bnc`）。

**2.4.0：词汇差距闭环。** 考试词表/词频入库 → 词汇量定级测试 → 重遇机制 →
蒸馏修正。设计依据是罗肖尼视频的两条规则（95–98% 覆盖率的可理解文本 + 在**变化的语境**中
重遇 ≥12 次），此前项目只做了半条。**接手先读 `docs/VOCAB-GAP-PLAN.md`**（含第一性原理、
四个断裂点、以及三轮调优的踩坑记录：不等宽档位放大噪声、伪词样本太小、词频挂在词元上）。
