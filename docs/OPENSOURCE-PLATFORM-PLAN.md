...
> 版本：v2（2026-09-16）—— 已按你的决策更新
>
> **已定：**
> - 真题 / 词表 / 短语沉淀 → **做成可选的下载包**，用户自己选装（不预置）
> - 移动端形态 → **PWA**（不做原生）
> - 移动端访问方式 → **Tailscale + 手机浏览器**（不做局域网二维码那套）
> - 移动端重点 → **交互逻辑 + 显示比例适配**（不是原生能力）
>
> **待定：** Apple/Windows 签名证书（影响桌面分发的体验）
...
1. ~~真题打包~~ **已定：做成可选下载包。**
   实现方式：`readloops packs` 子命令 + 设置页的「数据包」区块，逐个显示
   体积/来源/许可，用户勾选后再下载。默认**只装**可自由分发的那一部分。

   | 数据包 | 内容 | 体积 | 默认 | 许可 |
   |---|---|---|---|---|
   | **基础词表**（内置） | CET4/6/考研/雅思/托福词表 + 释义 | ~1–2MB | ✅ 预置 | ECDICT MIT |
   | 完整词典 | ECDICT 全量 18.6 万词 | ~63MB | ⬜ 选装 | ECDICT MIT |
   | 短语库 | 1037 条短语 + 已核实的释义 | < 500KB | ✅ 预置 | MIT / CC0 |
   | **真题语料** | 547 篇真题原文 | ~2MB | ⬜ 选装 | ⚠️ 无授权，**仅供个人学习**，界面上明确标注 |
   | 派生统计（内置） | 覆盖率曲线、风格基线 | < 20KB | ✅ 预置 | 事实数据 |

   这样既满足「打包给用户」，又把版权风险留在**用户自己的选择**里 ——
   默认安装不包含任何无授权内容。
