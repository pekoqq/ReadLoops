...
> 最后更新：2026-09-16（v2.6.5，修定级测试双击静默取消勾选的 bug）
...
### v2.6.5 — 修定级测试的致命交互 bug（已完成，2026-09-16；**未发布到 PyPI**）

用户重测得 2,200 词，但第 1 档仍只有 68.8%，且被标「不认识」的全是核心常用词
（`few`/`patient`/`certain`/`april`/`seat`/`radio`/`maintain`/`task`）。

**根因是勾选框的「切换」语义 + 双击**：Playwright 验证确认双击会触发两次 toggle，
把刚勾上的词又取消掉。用户勾了，数据里却没有。**这是设计错误，不是用户的错。**

→ 改成**两个显式按钮（认识 / 不认识），点击只设置状态、幂等**；状态改为
`Map<词, boolean>`；**本页未答完时下一页禁用**并显示剩余数，杜绝漏答。
验证：双击/连点均幂等、未答完禁用、答完解禁、无报错。

### v2.6.4 — 定级测试精度大修（已完成，2026-09-16；**未发布到 PyPI**）

用户实测 4,137 词，但**数字不可用**：95% 区间宽 2,950 词，同一份作答在 1,425~4,025 间摆动。
记录本身没 bug（模拟精确点击验证过），是**测试设计**问题：

- **估计量有 +1,000 词系统高估**：求和法把每档抽样噪声乘上档宽再加总，后段档真值近 0
  却被噪声抬到 5–25%，乘 16,000 词宽度 = 凭空多出上千词
  → 换成**逻辑斯蒂曲线拟合**取中点（2 参数拟合 20 点），偏差 +1000 → +70
- **每档只 8 题** → 16 题；**提示语「假词会揭穿猜测」逼用户少勾** → 改写并明确
  「常见的词请务必勾上，漏勾一个最常见档位的词会偏差上千词」

端到端：**MAE 630 → 183 词，稳定性 SD 350 → 63**。
